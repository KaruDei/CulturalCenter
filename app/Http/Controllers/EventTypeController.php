...
class EventTypeController extends Controller
{
    ...
    {
        return EventTypes::all()->toArray();
    }

    // Получение одной записи
    public function GetRecord($id)
    {
        return EventTypes::findOrFail($id)->toArray();
    }
    ...
    public function Create(Request $request)
    {
        $fields = $request->validate([
            'type' => 'required|min:1|max:255|unique:event_types',
        ]);

        try {
            EventTypes::create($fields);
    ...
    {
        $fields = $request->validate([
            'type' => 'required|min:1|max:255|unique:event_types,type,' . $id,
        ]);

        $record = EventTypes::findOrFail($id);
